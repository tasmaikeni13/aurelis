...
An element `(decay, increment)` represents the map `z ↦ decay • z + increment`.
`combine newer older` represents applying `older` first and `newer` second.
-/
...
/-- The empty block: no decay and no increment beyond the identity action. -/
def identity : Affine R X := ⟨1, 0⟩

/-- Chronological composition: `older` acts first, then `newer`. -/
def combine (newer older : Affine R X) : Affine R X :=
  ⟨newer.decay * older.decay,
   newer.increment + newer.decay • older.increment⟩

/-- Apply an affine scan element to an incoming state. -/
def act (element : Affine R X) (state : X) : X :=
  element.decay • state + element.increment

@[simp] theorem act_identity (state : X) : act (identity : Affine R X) state = state := by
  simp [act, identity]
...
/-- Sequential execution of chronological elements. -/
def run : List (Affine R X) → X → X
  | [], state => state
  | element :: later, state => run later (act element state)

/-- One bounded-size affine element representing an entire chronological list. -/
